...
- Supersedes: nothing. Scopes `.review/` usage for `ue-r2-*` only — the async sentinel (`ready/`) + `reviewer-wait.sh` hand-off is not used (records `findings/` + `results/` are reused); the `.review/` two-agent loop is unchanged for all other work. Records the owner's explicit, scoped relaxation of the §9 per-slice-code checkpoint (code acceptance delegated to this loop); the §8 push gate and all other §9 owner gates stand.
