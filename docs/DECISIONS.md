...
## D-2026-08-08-1 — v0.1.2 ships as a patch release; the 1.0 plan is deferred, not closed
- Decision: the next release is **v0.1.2**, cut from the current `master` head, and the workspace version reverts `1.0.0` → `0.1.2`. `docs/plan/RELEASE_1_0.md` stays **Active** with G2–G6 open; **not one of its gates is closed, satisfied, or waived by this release**. A future v1.0.0 still has to close them on its own candidate.
- Why: owner ruled "blit 0.1.2" (2026-08-08) when asked directly whether to cut 1.0.0 or a real 0.1.2 patch, deliberately setting the 1.0 plan aside. `1.0.0` had been staked on 2026-08-02 (`ef9a13b2`) as a *candidate* label for that plan, and G5c alone (BLIT_CONSOLE C1–C4 + T1–T3, D-2026-08-04-3) is months of work; shipping the landed reliability and performance work should not wait on it.
- Scope warning, load-bearing: **reverting the version LABEL reverts no code.** The 0.1.2 candidate carries every commit since `v0.1.1` (177 of them) — per-file error containment, the ls-0..6 compare/apply performance work, the CLI live progress row, CI code signing, and the D-2026-08-01-4 compare-contract change. A future reader must not infer a small diff from the small version number; `CHANGELOG.md` `[0.1.2]` is the authoritative scope statement, not the version bump.
- Supersedes: nothing. D-2026-08-02-1, D-2026-08-04-3 and RELEASE_1_0's gates are untouched and still bind v1.0.0. `ef9a13b2`'s `1.0.0` candidate label is retired; the interim `dist/` archives built off it are now historical pipeline proof only, since no shipped binary will report `1.0.0`.
- Open, deliberately not fixed here: `docs/RELEASE_NOTES_1_0.md` is written as 1.0's notes and dates the compare-contract change "as of 1.0". It is a 1.0-plan artifact (G5, open), so this ruling leaves it alone rather than half-editing an open gate's deliverable — but it now describes changes that ship in 0.1.2, and re-cutting it belongs to whoever closes G5.

## D-2026-08-12-1 — Package managers ship both signed archives and source rebuilds
- Decision: brew / AUR / winget / Scoop are **both** lanes: wrap the signed (or Linux-unsigned) GitHub Release archives, **and** offer source rebuilds where the manager has a source-package convention (Homebrew core formula, AUR source package). Cargo stays a later decision (plan D3).
...
## D-2026-08-15-2 — 1.0 is CLI + daemon only; no UI gate on any release
...
## D-2026-08-17-1 — Interface-platform direction: one SDK layer, standalone front-ends
- Decision: `docs/plan/INTERFACE_PLATFORM.md` is the direction (R1 approved): exactly one programmatic interface to the engine — a single headless SDK crate consolidating `blit-app` + `blit-console-core` — with the CLI as its reference consumer, and TUI/GUI built later in `BlitAdmin_UIs` as standalone, independently installable, daemon-capable first-class SDK consumers. `docs/plan/UI_REMOVAL.md` is superseded before ever activating.
- Why: owner, 2026-08-17: UIs "need to be first-class interfaces to the blit engines," each working standalone (`winget install blit-gui`) and independently of the others; the CLI is just the first-shipping UI. Deleting the console layer as UI residue would have destroyed the layer that makes that possible.
- Supersedes: `docs/plan/UI_REMOVAL.md` (Draft, never activated) entirely; amends D-2026-08-15-1 — "no TUI, no GUI in this repo" stands, but the headless interface layer is a first-class product of this repo, not UI residue to delete. D-2026-08-15-2 (1.0 gates on CLI + daemon only) unchanged. Plan status stays Draft pending R2–R4 (SDK crate name, `blit-tui`/`blit-gui` dispositions); no code until Active + per-slice go.

## D-2026-08-17-2 — no separate interface crate: everything folds into blit-core; blit-core publishes to crates.io
- Decision: `blit-app` and `blit-console-core` fold into `blit-core`, which is the single programmatic interface for every front-end (CLI, future TUI/GUI in BlitAdmin_UIs) and for third-party Rust apps embedding Blit; non-Rust integration is the daemon's gRPC surface (`proto/blit.proto`). `blit-core` will be published to crates.io (name verified available 2026-08-17; the publish act itself stays owner-gated). INTERFACE_PLATFORM ruling R2 (interface-crate name) is dissolved — there is no new crate to name.
- Why: owner, 2026-08-17: "why can the console stuff not live in blit-core?" — no defensible reason exists (core already carries the gRPC client type and mDNS discovery; the dependency delta is one crate, chrono, plus a sysinfo feature flag). On publication: "I never deliberated or proclaimed anything like that. it should be published" — the plan's crates.io non-goal was agent-invented, never an owner ruling.
- Supersedes: INTERFACE_PLATFORM.md's original merged-SDK-crate shape (slices if-1..if-3) and its crates.io non-goal; the plan is reworked in place this session. D-2026-08-17-1's direction (one interface layer, standalone out-of-repo front-ends, CLI as reference consumer) is unchanged — the layer is now blit-core itself.

## D-2026-08-17-3 — blit-prometheus-bridge is removed
...
## D-2026-08-18-1 — cargo IS a distribution channel; the CLI publishes as blit-transfer
- Decision: crates.io becomes a supported install channel: `cargo install blit-transfer` (crate renamed from `blit-cli`; the produced binary stays `blit`, directory stays `crates/blit-cli`) and `cargo install blit-daemon`, with `blit-core` published as their dependency. Names verified free 2026-08-18 (`blit`/`blit-cli` are squatted by unrelated crates, which is why the crate name and command name differ). Source-lane package templates and platform scripts now build `-p blit-transfer`. The publish acts remain owner-gated.
- Why: owner, 2026-08-18: asked for cargo installability "like cargo install topgrade"; on the name, "blit-transfer works."
- Supersedes: D-2026-08-12-4 (cargo is not a user channel) and the pm plan's derived prohibitions (README may advertise `cargo install blit-transfer` once the crates are live and queryable — same only-live-channels rule as every other channel; the plan text is edited in place this session).

## D-2026-08-18-2 — protocol version is the session gate; build identity becomes informational
- Decision: session open refuses on `contract_version` mismatch only ("protocol version is the key" — owner, 2026-08-18). `build_id` stays in the hello, faults, and diagnostics as information, never a veto. Corollary made load-bearing: any wire-shape or wire-behavior change bumps `CONTRACT_VERSION` in the same commit. Additionally, discovered daemons surface their protocol version — mDNS TXT `contract` property, shown by `blit scan` and carried in the endpoint model for future TUIs/GUIs. Execution: `docs/plan/CONTRACT_VERSION_GATE.md` (cv-1..cv-3).
- Why: the same-build rule was protective scaffolding while the wire churned; it now blocks the shipped channels from interoperating (every cargo build carries a unique `unknown.<nonce>` identity, so plain cargo installs refuse ALL peers), and it would block independently-versioned front-ends and third-party embedders (D-2026-08-17-2).
- Supersedes: D-2026-07-05-2 (same-build peers only, refusal at session open) once cv-1 lands; until then the old gate remains the shipped behavior. Fault code `BuildMismatch` keeps its wire tag; message rewords.

## D-2026-08-20-1 — perf history must be useful before the next release
- Decision: the next release tag is BLOCKED until perf data does both jobs ruled "both" by the owner: feed planning (warm-started adaptive dials from recorded history — local runs, remote runs, daemon-served runs, per machine) and report honestly (`blit profile`/`blit diagnostics perf` show every real transfer, route-labeled, per-route aggregates). Plan: `docs/plan/PERF_HISTORY_PLANNING.md` (Draft; open rulings R1–R3 there).
- Why: verified 2026-08-20 that the daemon records nothing, remote runs record nothing anywhere, and nothing plans from the history — the predictor's only consumer is the `blit profile` display; owner: "we need to make the perf data actually useful."
- Supersedes: the report-completeness-only reading of Post-REV4 residue item (2); the tag-sequencing note in the 2026-08-19 handoff (rig runs → ruling → tag) gains this plan as a prerequisite.

## D-2026-08-20-2 — perf-history design rulings delegated; R1–R5 resolved
- Decision: owner delegated the perf-history design calls ("you're designing this. the principles are 'FAST, SIMPLE, RELIABLE file transfer' don't violate that"; on the daemon-history surface: "it doesn't matter to me. whatever."). Resolved under that bar: R1 — retire the gradient-descent predictor in favor of the settled-dial seed store; R2 — fleet/wire exposure of history stays deferred; R3 — ph-6's rig A/B demonstration is the plan's closing acceptance bar; R4b — worker warm-start is an accelerated post-open ramp, wire frozen, CONTRACT_VERSION untouched; R5b — `blit profile`/`blit diagnostics perf` merge daemon-recorded history with origin labels (chosen over the owner-floated `blit-daemon profile` alternative — one truth-telling surface, no new command).
...
## D-2026-08-20-4 — ph-6 rig proof accepted; PERF_HISTORY_PLANNING ships; release blocker cleared
- Decision: owner ruled "pass" on the R3 acceptance bar over the clean A/B evidence (`docs/bench/PERF_HISTORY_PLANNING/`): warm == cold within noise both directions, seeds persist/reuse/settle per class (fwd `workers:4`, rev `workers:1`), merged `[daemon]`/`[operator]` reporting verified live, win ≈ 0 on this rig because the dial settles within the first epochs even cold. `docs/plan/PERF_HISTORY_PLANNING.md` → **Shipped**; the D-2026-08-20-1 release-tag block is cleared.
- Why: the plan's demonstrated claims are seed correctness, reuse, and absence of warm-start regression — R4's design rationale predicted the near-zero win when settle time is already small; the owner accepted the honest result over a manufactured speedup fixture.
- Supersedes: the "Awaiting owner pass/fail" state recorded in the plan's ph-6 section and `docs/STATE.md` 2026-08-20; the next-tag block in D-2026-08-20-1 (its reporting/planning requirements are met and proven).
