...
Last updated: 2026-07-15 (P2 observer locally guard-proved; Claude review next; no causal grade)

- **NEXT ACTION — CLAUDE-REVIEW THE P2 OBSERVER, THEN THE PINNED HISTORICAL CONTROL:** The default-off aggregate P2 observer is implemented and locally guard-proved across both initiator roles and TCP/in-stream OFF/ON; `.review/findings/otp12-pf1-p2-observer.md` owns its exact schema, eight mutation proofs, and operational constraints. It has produced no performance evidence and cannot grade H6/H7. After Claude Fable 5/max accepts the exact committed slice, adapt the neutral schema to pinned `0f922de` and run the plan's local old/new + observer-OFF/ON TCP/gRPC control. In-stream correlation requires one shared run ID per observed session; the existing block-level rig harness is not valid unchanged. If local P2 is null, rig-W plus zoey fallback still applies. Further P1 rig time needs an owner-approved amendment.
