...
Last updated: 2026-07-21 (exact ldt-4 d53b5fd staged/self-tested; live preflight blocked by Windows DHCP drift .177→.173)
...
- **NEXT ACTION — RESTORE NETWATCH-01 TO REGISTERED `.177`, THEN RUN EXACT `d53b5fd`:** q's `en8`/`.54` is restored at MTU 9000/10Gbase-T with the Windows route on `en8`. Exact `d53b5fd` is mutation-proved, tactical Grok-clean, additively staged, and q Bash 3.2 self-test green. Its first attached launch failed closed before evidence reservation because pinned `10.1.10.177` is absent while strict identity on nagatha proves `NETWATCH-01` now owns `.173`. Restore the registered address to preserve the reviewed harness; changing the pin requires a separately planned and reviewed code slice.
...
- **LIVE_DIAL_TUNING ACTIVE (D-2026-07-16-2):** ldt-1 is accepted at `f8f3c51`; ldt-2 is accepted at `65a0f9f`; ldt-3 lifecycle/observer closure is accepted at `406a7e5`; the ldt-4 rig-W endpoint correction is accepted at `9926bf7`. Repairs through `a39f0c5` are tactical Grok-clean; the fresh run exposed separate `start.cmd` array finding `ldt-4-live-f4`, fixed, mutation-proved, tactical Grok-clean, and additively staged at `d53b5fd`. q's registered 10 GbE link is restored; the live retry now waits for Windows to return from DHCP `.173` to harness-pinned `.177`. Formal Fable openreview is held for capacity.
...
1. **`docs/plan/LIVE_DIAL_TUNING.md` (ACTIVE, D-2026-07-16-2).** ldt-1..3 and endpoint correction `9926bf7` are accepted. Repairs through `a39f0c5` are guarded and tactical Grok-clean. Its additive retry cleared fixtures and environment, then exposed generated start-command finding `ldt-4-live-f4` before daemon launch or timing. The fix at `d53b5fd` is mutation-proved with full local gates green, tactical Grok-clean, additively staged, and q Bash 3.2 self-test green. Formal Fable openreview is held; restore Windows' registered `.177` identity, then execute fresh quiet rig-W `q`↔`netwatch-01` adaptive and role-invariance evidence.
...
- **ldt-4 rig-W endpoint (as of `3c5084e`, 2026-07-21):** q's registered `en8`/`.54` link is restored at MTU 9000/10Gbase-T and routes to Windows on `en8`. Harness-pinned Windows `.177` has incomplete ARP and no TCP/22, while DNS plus strict-host-key SSH from nagatha prove `NETWATCH-01` now owns `.173`. The exact harness stopped before evidence reservation. Restore `.177`, or separately plan/review an endpoint-pin change; never bypass host-key checking or substitute an unreviewed address.
