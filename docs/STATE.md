...
Last updated: 2026-07-15 (pf-1 rig-W G11 fixed/guard-proved; external review next; no accepted rig datum)

- **NEXT ACTION — PF-1 RIG-W G11 EXTERNAL REVIEW:** exact candidate `5a7e7ec` passed launcher smoke and preflight, then its first registered arm voided before durability/CSV append: three sequential SSH clock probes consumed about 1.1 s and produced a 1247 ms settle. G11 carries the same three independently bracketed samples over one bounded channel, preflight-times the complete recording path, and is mutation-proved red against the old implementation. Run external Grok or Claude review, then stage a new exact candidate additively and retry launcher, preflight, and the registered q↔netwatch-01 run. The worker-count disparity is closed and is not a blocker; no datum has been accepted or graded.
...
1a. **`docs/plan/OTP12_PERF_FINDINGS.md` (ACTIVE, D-2026-07-13-1).**
    ...
    time. Round-10 independent Grok accepted exact candidate `5a7e7ec`. Its
    launcher and preflight passed; the first registered arm then voided at
    1247 ms settle before durability or CSV append because three sequential
    clock-probe SSH startups consumed about 1.1 s. G11 batches the unchanged
    three independently bracketed samples, fails closed on channel/process
    errors, and preflight-times the complete path against 750 ms headroom.
    External Grok or Claude review is next; then a new exact candidate, additive
    staging, and the registered retry. The phase report and
