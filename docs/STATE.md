...
- **NEXT ACTION — REVIEW `ldt-4-live-f14`:** the analyzer now requires exact protobuf SOURCE actions while preserving dial shorthand. The 88-test guard is mutation-proved, and a diagnostic copy of the retained void session validates all four arms as `REVIEW_REQUIRED` (arm 3, decision 1, performance 0). Run tactical Opus 4.8/max, then restage and rerun additively.
