...
- **NEXT ACTION — COMPLETE THE THREE ANALYZER LOW FIXES, ONE PER COMMIT:** Fable r1 reviewed exact `e41b871..0e48721`; intake and per-finding status live in `.review/results/ldt-4-harness-r1.claude-verdict.md` and `.review/findings/ldt-4-r1-*.md`. Medium `f1`–`f3` and the narrowed comment-truth `f5a` are fixed and guard-proved. Add Python replay branch guards, reject duplicate trace JSON keys, and require exact blocked-ratio recomputation; then run neutral whole-change Fable review before endpoint staging or measurement.
