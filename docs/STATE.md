...
0. **ULTRACODE 2026-08-18 (DEVLOG 23:00Z): cv-1+cv-2 LANDED `05529c19` (contract-version gate + scan surfacing; cv-3 waits for next release); sf-3d LANDED `7ffd929d` (containment cache + resume-path stamping; rig A/B still owed); clp-3 reviewed (F1 fix agent in flight; F2/F5 owner calls); residue triaged (2 dead, 1 dead-as-written, 2 need owner rulings); sweep-prefetch parked pending one netwatch-01 SMB run; pm-5 BLOCKED for v0.1.2 (tag predates pm-1) — source lane needs the next tag.** Combined gate green 1182/0/2. UNPUSHED from `81123d14` onward.
