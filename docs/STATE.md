...
- **NEXT ACTION — COMPLETE THE TWO EVIDENCE-PARSER LOW FIXES, ONE PER COMMIT:** Fable r1 reviewed exact `e41b871..0e48721`; intake and per-finding status live in `.review/results/ldt-4-harness-r1.claude-verdict.md` and `.review/findings/ldt-4-r1-*.md`. Medium `f1`–`f3`, comment-truth `f5a`, and Python replay coverage `f6` are guard-proved. Reject duplicate trace JSON keys and require exact blocked-ratio recomputation; then run neutral whole-change Fable review before endpoint staging or measurement.
