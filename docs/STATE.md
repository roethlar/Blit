...
Last updated: 2026-07-15 (pf-1 rig-W live G13 fixed/guard-proved; Claude review next; no accepted rig datum)

- **NEXT ACTION — PF-1 RIG-W G13 CLAUDE REVIEW:** exact `d5e9dda` was built/staged additively and passed q Bash 3.2 self-test, launcher, and preflight. Its registered run cleared G12 by completing all eight pair-1 arms, then became `SESSION-VOID` before pair 2 because the mid-run one-minute q load gate counted 74 seconds of the benchmark's own work (1.79 before, 3.19 after) as external load. G13 keeps the fixed 3.0 ceiling and every check, adds bounded runtime recovery outside timed arms, leaves preflight immediate, and guard-proves persistent load still fails closed. Complete validation is green; run the required Claude Fable 5/max review, then build/stage a fresh exact candidate additively and retry. Worker parity remains closed and is not a blocker.
...
1a. **`docs/plan/OTP12_PERF_FINDINGS.md` (ACTIVE, D-2026-07-13-1).**
    ...
    benchmark's own one-minute load history; that session is also void. G13 keeps the
    fixed load ceiling with bounded runtime recovery and is Bash-3.2 guard-proved;
    complete validation is green and Claude review is next. The phase report and
