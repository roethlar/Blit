...
Last updated: 2026-07-21 (valid ldt-4 rig-W evidence retained; adaptive horizon finding next)
...
- **NEXT ACTION — OWN THE ldt-4 ADAPTIVE-HORIZON FINDING:** exact `96a4e3b` completed all 96 valid rig-W arms and its retained analysis was independently reproduced byte-for-byte. Every arm stayed at four streams: 74 arms ended before any tuner sample and 22 produced one, so the matrix did not prove adaptive ADD/REMOVE behavior. Add a dedicated sustained controller-exercise workload without replacing the existing performance cells; treat the two performance-review cells as a separate later finding.
...
- **LIVE_DIAL_TUNING ACTIVE (D-2026-07-16-2):** ldt-1..3 are accepted. Exact `96a4e3b` completed all 96 valid rig-W arms; retained evidence and independently reproduced analysis are `REVIEW_REQUIRED`, not acceptance. The fixtures ended before any adaptive membership change, exposing the next controller-horizon finding; two performance-review cells remain separate. Formal Fable openreview is held for capacity.
