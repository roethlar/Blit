...
1.0 archives in dist/ off `ef9a13b2`; audit-18/19 ship documented unless owner overrides. CI `build-release` now signs the shipped binaries when the signing secrets are present (macOS codesign + notarize, unstapled by design; Windows Azure Trusted Signing) and packages unsigned when they are not; the signed path is proven green end to end (run 31229611237, DEVLOG 2026-08-08). A `v*` tag push now runs that same gate and each matrix leg attaches its archive + `.sha256` to that tag's GitHub release, non-draft as v0.1.0/v0.1.1 were — no longer a by-hand upload, but unexercised until the first real tag (DEVLOG 2026-08-08).
