...
- **NEXT ACTION — FINAL FABLE REVIEW, THEN A FRESH EXACT LIVE LAUNCH:** The first exact `4e0fdc3` launch staged the registered artifacts and small fixture, then voided before any arm or runtime swap because the endpoints' large fixtures differed in content. `b0c6ce3` stages canonical Windows large/mixed fixtures through retained per-session incoming paths into stable q source paths; mutation guards and full local gates pass. Review the exact candidate before another launch.
