...
- **NEXT ACTION — an OWNER JUDGEMENT CALL, then round 8. ROUND 7 IS IN: both reviewers again say NOT READY** (`.review/results/macmac-harness-r7.{codex,grok}.md` — **read them first**). Instrument at `1e03063`, prereg **rev 7**. Real new findings: the **drain fails open again** (if `hrun` prints `drained_*` and *then* exits non-zero, `|| echo DRAIN-ERROR` yields a **two-line** value that still starts with `drained` — my own round-5 rc fix); **prereg rev 7 now contradicts itself** (older sections still describe bar-based materiality and null-only blocking); **`CONTROLS-UNCERTIFIED` advises a `RUNS=16` escalation the harness will not authorise** (it only accepts `INCONCLUSIVE-UNDERPOWERED`); the settle can still be **shadowed via PATH/BASH_ENV**; and grok's **dead zone** — the half-margin may make the controls *impossible* to certify, which would make the rig unable to ever conclude. **THE JUDGEMENT CALL: seven rounds, 80+ findings, zero data. The decision RULE — not the measurement — is now where the bugs live. Continue hardening, or radically SIMPLIFY** (keep the pre-registered question, the paired statistic and a fixed reporting format; drop the baroque multi-outcome taxonomy the owner walks anyway)? The harness **REFUSES a timed run** (`exit 2` without `CLEARED_BY_REVIEW=1`) and **NOT ONE DATUM HAS EVER BEEN TAKEN.** **⚠ TO RUN IT THE OWNER MUST CLOSE THEIR CODEX SESSIONS** — nagatha is a bench **END**; the quiescence gate refuses while `codex`/`cargo`/`rustc` runs on **either** Mac (it fires correctly today, and load on nagatha is what trips it). Time Machine OFF on both. Rig: nagatha `10.1.10.92` ↔ `q` `10.1.10.54`, 10GbE/9000, build pinned `f35702a`. **Then `pf-1`** (the HARD GATE), which two pf-0 results BIND: between-session grading is dead (a 20% recovery = 46 ms sits under the 78 ms floor), so pf-1 must **measure its own paired within-session floor** before grading; and the fast arm is **BISTABLE** — grade the distribution, not the median.
