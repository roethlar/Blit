...
- **NEXT ACTION — ROUND 8 IS IN (both reviewers: NOT READY) and it found a REAL HOLE IN THE SIMPLIFICATION, not a patchable branch.** Instrument at `79c1f2d`, prereg **rev 8** (D-2026-07-14-3, owner: "simplify"). Read `.review/results/macmac-harness-r8.{codex,grok}.md` first. **NO DATA EVER TAKEN**; the harness still refuses a timed run.
  - **BLOCKER — deleting `UNSTABLE` was UNSOUND at n=16.** The rewrite argued "a bimodal arm widens the CI, so a wide CI lands in UNCLEAR". **That is true at n=8 (where the CI IS the full range) and FALSE at n=16**, where the interval is `[d(4), d(13)]` and therefore **trims three outliers per side** — so a bimodal arm can yield a *narrow* CI (codex drove `[1,1]`) and a **false null**. **THE DESIGN TENSION, which needs a decision, not a patch:** an equivalence claim must not be reachable by *trimming away the very pairs that contradict it*, so `NONE` arguably requires the **full range** inside ±T — but the full range only *widens* with n, which means **the `RUNS=16` escalation cannot help a null at all**, and its whole purpose was to rescue an `UNCLEAR` rig. Either the escalation is worth little and should go (a noisy rig is fixed by a quieter rig, not more pairs), or `NONE` needs an outlier-aware rule that is not simply "trim". **Decide this before writing more code.**
  - **BLOCKER — a "clean" control's residual bias is not carried into the measurand.** A control at `+49` with `T/2 = 50` certifies clean, yet that 49 ms of arm bias may ride in the measurand too — so a measurand `EFFECT` at 100 could be 51 real + 49 rig.
  - **BLOCKER — the drain STILL fails open.** My round-7 fix checks the *value* but the `hrun` exit status is discarded inside the command substitution, so a drain that printed `drained_*` and then FAILED is still accepted.
  - **HIGH — the controls are not contemporaneous**: both measurand cells run first, then the controls, so a transient can hit one and not the other. The controls cannot certify a window they did not share.
- **THE INSTRUMENT IS THE RISK — 69 findings across SIX reviews of this ONE harness, 69 accepted, 0 rejected, and it has still never run.** Three project claims were already retracted to harness bugs. **TWO DEFECT CLASSES recurred in EVERY round; the next review must assume both are still present.** (1) **"Fixed the branch I was shown, not the class"** — the *same* materiality bug escaped **four** rounds running, each through a branch the last fix missed (measurand → control-`PARTIAL` → control-`UNDERPOWERED` → the marginal bar substituting for paired magnitude, reporting `REPRODUCES` off a **1 ms** effect); a fail-open `pgrep` was fixed in one gate and left in its duplicate. **Rev 7's answer is structural**: the 1.10 bar takes **no part in inference** (acceptance criterion only); direction = sign test, magnitude = paired CI, equivalence = CI vs margin; and **the controls are a PRECONDITION** — unless each certifies below **half** the material effect, **no verdict is read, neither null nor reproduction**. (2) **"A fix that never executed"** — `SETTLE_MS`, introduced to neutralise an artifact **capable of manufacturing a one-directional P1**, **HAD NEVER RUN in any revision** (a quoting bug killed the `sleep`, and its exit status was discarded), while the prereg asserted it for **three revisions**. It is now **measured inside the same process as the fsync walk** (`settled_ms`); a pair that did not settle **VOIDS**. Earned rules: **verify the instrument before believing the measurement**; **`bash -n` is not an execution**; **read the existing harnesses first**; **a protection that cannot be observed is not a protection.**
