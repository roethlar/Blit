...
Last updated: 2026-07-15 (exact 8e019ef rig-W run accepted; live eight-stream parity; P1 causal grade unresolved)

- **NEXT ACTION — REVIEW THE PF-1 RIG-W PROBE RECORD, THEN CONTINUE P2:** Exact Claude-accepted `8e019ef` passed fresh staging, q Bash 3.2 self-test, SSH-driven launcher smoke, preflight, and registered session `20260715T211759Z.30531`. The analyzer accepted the complete 128-arm OFF–ON–ON–OFF schedule; exact evidence is under `docs/bench/otp12-pf1-rigw-2026-07-15/`. Every traced target arm reached eight live streams under both initiator layouts. Historical P1 did not reproduce and reversed within pointwise 1.10, but `N_resolution=329 ms` exceeds the historical gap and the gRPC control fails, so no cause is graded and P1 is not formally closed. Claude Fable 5/max review of the immutable probe record is next; then the already-approved P2 small-fixture instrumentation and `0f922de` historical control can proceed. Further P1 rig time needs an owner-approved plan amendment for the absent/reversed baseline and adequate resolution.
...
1a. **`docs/plan/OTP12_PERF_FINDINGS.md` (ACTIVE, D-2026-07-13-1).**
    ...
    G15 isolates SSH stdin and is mutation-proved at `7bdaf8b`; round-15 Claude accepted exact `8e019ef`. Its additive registered run completed all 128 arms and the analyzer accepted the exact evidence now recorded under `docs/bench/otp12-pf1-rigw-2026-07-15/`. Live target traces prove 8/8 stream parity under both initiator layouts. The target's historical P1 direction did not reproduce, but the 329 ms resolution floor and failing gRPC control forbid a causal grade. Review that probe record, then continue the separately required P2 small-fixture instrumentation and `0f922de` historical control. Further P1 rig work requires a plan amendment; no Mac↔Mac data has been taken, and worker parity is no longer a blocker. Then: pf-1 → pf-final (all rigs) → otp-12d → otp-13.
