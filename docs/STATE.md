...
- **NEXT ACTION — CONTINUE THE ADMITTED ldt-4 REVIEW FIXES, ONE PER COMMIT:** Fable r1 reviewed exact `e41b871..0e48721`; intake and per-finding status live in `.review/results/ldt-4-harness-r1.claude-verdict.md` and `.review/findings/ldt-4-r1-*.md`. Tagged strict Windows fetch framing and hard-crash daemon-baseline refusal are fixed and mutation-proved. Complete the remaining fixes, then run neutral whole-change Fable review before any endpoint staging or measurement.
