...
|-------------------|----------|---------------------------------------------|--------|-------------|-----------|
| audit-h1-mirror-relay-incomplete-scan | Data-loss | Reject `mirror --relay-via-cli` for remote→remote (the relay scanner discards `unreadable_paths`, letting mirror purge run on an incomplete source view); symmetric with `move --relay-via-cli` R50-F1/R51-F2 | `[~]` | `master` | `c6b3a3a` |
| audit-h3a-push-receive-stall | Robustness | StallGuard on the daemon push-receive socket (`TRANSFER_STALL_TIMEOUT` hoist) — closes one of three remaining stall-guard gaps from R3 H3; symmetric with audit-1c CLI pull-receive | `[~]` | `master` | `dd51a1c` |
| audit-m28-tui-sot-sweep | Docs | TUI source-of-truth sweep: `docs/plan/README.md` points at TUI_REWORK as active + audit INDEX as open-finding source; TUI_DESIGN.md banner'd SUPERSEDED; ARCHITECTURE.md §"blit-tui" rewritten to lead with the dual-pane M1–M6 plan | `[~]` | `master` | `e8a5280` |
| audit-l39-m27-env-var-purge | Convention | Owner-directed: env vars OUT for app + diagnostic config. `BLIT_TUI_INPUT_TRACE` → `--trace-input`; `BLIT_TEST_COUNTER_FILE` → `--diagnostics-counter-file`; documented overrides `BLIT_FORCE_GRPC_DATA` + `BLIT_DISABLE_LOCAL_TELEMETRY` struck from greenfield_v6 §1.2/§1.3 | `[~]` | `master` | `7c3ffd5` |
