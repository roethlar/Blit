...
- [ ] **Phase 5 — Milestone M-Jobs.** Daemon-owned transfer lifecycle for **remote→remote transfers only** (push/pull tie the CLI to the byte path; detach is architecturally impossible there). Adds `detach: bool` field on `DelegatedPullRequest`, `CancelJob` RPC, per-job event ring, `CancellationToken` field on each `ActiveJob` row, `transfer_id_filter` on `SubscribeRequest`. When `detach=true` on a delegated pull, the daemon disarms the `tx.closed()` cancellation race and owns the transfer to completion or `CancelJob`. CLI gains `--detach` flag (accepted on remote→remote only — clean error on push/pull) plus `blit jobs cancel` and `blit jobs watch` admin verbs. See `TUI_DESIGN.md` §6.5.
