...
- [ ] **P0** Remote transfer parity refactor (see `docs/plan/REMOTE_TRANSFER_PARITY.md`):
    ...
    - [ ] Implement multiplexed TCP data-plane streams (client + daemon) driven by auto-tuned worker counts so push/pull saturate 10 GbE. *(WIP: design in progress; current implementation still single-threaded and bottlenecked ~500 Mbps.)*
