...
async fn next_data_plane_header(
    files: &mut mpsc::Receiver<FileHeader>,
    cache: &mut HashMap<String, FileHeader>,
    rel: &str,
) -> Result<FileHeader, Status> {
    if let Some(header) = cache.remove(rel) {
        return Ok(header);
    }

    while let Some(next) = files.recv().await {
        if next.relative_path == rel {
            return Ok(next);
        } else {
    ...
    Err(Status::invalid_argument(format!(
        "unexpected data plane file '{}' with no manifest entry",
        rel
    )))
}
...
async fn read_i64(stream: &mut TcpStream) -> Result<i64, Status> {
    let mut buf = [0u8; 8];
    stream
        .read_exact(&mut buf)
        .await
