...
//! gates. The substantive logic lives in `blit_core::endpoints`.
...
//! `blit_core::endpoints` directly at each call site rather than
...
/// Library counterpart: [`blit_core::endpoints::ensure_remote_pull_supported`].
pub fn ensure_remote_pull_supported(args: &TransferArgs) -> Result<()> {
    blit_core::endpoints::ensure_remote_pull_supported(args.dry_run, args.workers.is_some())
}

/// Clap-shaped wrapper for the push gate. `--checksum` passes since
/// otp-10b-2 (the session's Checksum compare is role-agnostic).
/// Library counterpart: [`blit_core::endpoints::ensure_remote_push_supported`].
pub fn ensure_remote_push_supported(args: &TransferArgs) -> Result<()> {
    blit_core::endpoints::ensure_remote_push_supported(args.dry_run, args.workers.is_some())
}
