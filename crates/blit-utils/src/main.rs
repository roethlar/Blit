...
use blit_core::generated::{FileInfo, ListModulesRequest, ListRequest};
...
enum Commands {
    ...
    Df(DfArgs),
    /// Remove files/directories remotely (not yet implemented)
    Rm(RmArgs),
...
async fn main() -> Result<()> {
    ...
    match cli.command {
        ...
        Commands::Rm(_args) => {
            bail!("`blit-utils rm` is not implemented yet (Phase 3 task)");
        }
